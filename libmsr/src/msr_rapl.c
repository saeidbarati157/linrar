...
int poll_rapl_data(const unsigned socket, struct rapl_data ** result)
{
    ...
    p = &(rapl[socket]);
    fprintf(stderr, "Im here 1, before read_rapl_data");
    read_rapl_data(socket); //, p);
    fprintf(stderr, "Im here 2, after read_rapl_data, before delta_rapl_data");
    delta_rapl_data(socket, p, result);
    fprintf(stderr, "Im here 1, after delta_rapl_data");
...
//#endif
    fprintf(stdout, "at the end of read_rapl_data");
